...
| [技术架构方案（Wiki）](wiki/technical-architecture.md) | 系统分层、目录边界、主链路、数据/LLM/API/通知/部署架构与扩展点 |
| [如何分析一只股票（Wiki）](wiki/stock-analysis.md) | 单股分析需要哪些信息、端到端流水线、技术面/情报/LLM 输入分层与决策仪表盘产出概览 |
| [数据获取接口统计（Wiki）](wiki/data-fetch-interfaces.md) | 日线/实时/筹码/基本面/搜索等 Manager 入口、Fetcher 能力、底层 API 与配置速查 |
...
| 文档 | 内容 |
| --- | --- |
| [技术架构方案（Wiki）](wiki/technical-architecture.md) | 逻辑分层、核心流水线、子系统边界与扩展点 |
