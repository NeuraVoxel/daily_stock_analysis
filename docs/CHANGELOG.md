...
## [Unreleased]
- [修复] `market-flow` 在 Docker 内东财资金流接口被普通 `requests` 断连导致 `/api/flow` 503：改用 `curl_cffi` 浏览器指纹拉取并加重试与错误日志。
- [修复] `market-flow` 东财 `push2` 不可达时自动降级到同花顺行业资金流，避免 Docker UI 持续 503。
